<?php
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			$data['giatreem'] = (isset($data['giatreem']) && $data['giatreem']!=='') ? str_replace(",","",$data['giatreem']) : 0;
			$data['gianguoilon'] = (isset($data['gianguoilon']) && $data['gianguoilon']!=='') ? str_replace(",","",$data['gianguoilon']) : 0;
			$data['giaembe'] = (isset($data['giaembe']) && $data['giaembe']!=='') ? str_replace(",","",$data['giaembe']) : 0;
			$data['socho'] = (isset($data['socho']) && $data['socho']!=='') ? str_replace(",","",$data['socho']) : 0;
	...
	function save_list()
	{
		...
		if($id)
		{
			...
			if(isset($_FILES['file_icon']))
			{
				...
				if($icon = $func->uploadImage("file_icon", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_icon))
				{
					$data['icon'] = $icon;
					$row = $d->rawQueryOne("select id, icon from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
			$data['ngaysua'] = time();
	...
	function delete_list()
	{
		...
		if($id)
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo,icon from #_product_list where id = ? and type = ? limit 0,1",array($id,$type));

			if($row['id'])
			{
				$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				$func->delete_file(UPLOAD_PRODUCT.$row['icon']);
