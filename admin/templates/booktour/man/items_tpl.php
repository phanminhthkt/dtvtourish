<?php
	$linkMan = $linkFilter = "index.php?com=booktour&act=man&p=".$curPage;
    $linkEdit = "index.php?com=booktour&act=edit&p=".$curPage;
    $linkDelete = "index.php?com=booktour&act=delete&p=".$curPage;
...
<section class="content">
    ...
    <?php if(isset($config['booktour']['search']) && $config['booktour']['search'] == true) { ?>
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title">Tìm kiếm đặt tour</h3>
            </div>
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title card-title-order d-inline-block align-middle float-none">Danh sách đặt tour</h3>
            <?php if(isset($config['booktour']['excelall']) && $config['booktour']['excelall'] == true) { ?>
            ...
            <?php } ?>
            <?php if(isset($config['booktour']['wordall']) && $config['booktour']['wordall'] == true) { ?>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="align-middle">Mã đặt tour</th>
                        ...
                        <?php if(
                            (isset($config['booktour']['excel']) && $config['booktour']['excel'] == true) || 
                            (isset($config['booktour']['word']) && $config['booktour']['word'] == true)
                        ) { ?>
                    ...
                    <tbody>
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
	                            ...
	                            <?php */ ?>
                                <td class="align-middle">
                                    <a class="text-primary" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$items[$i]['matour']?>"><?=$items[$i]['matour']?></a>
                                </td>
                                <td class="align-middle">
                                ...
                                <td class="align-middle">
                                    <span class="text-info">
                                        <?php 
                                            if($items[$i]['status_vnpay']!=''){
                                                echo '<span class="d-block">Thanh toán online</span>';
                                                if($items[$i]['status_vnpay']==1){
                                                    echo '<span class="d-block text-purple">Trạng thái: Thành công</span>';
                                                }else{
                                                    echo '<span class="d-block text-danger">Trạng thái: Lỗi</span>';
                                                }
                                            }else{
                                                echo $func->get_payments($items[$i]['httt']);
                                            }
                                        ?>
                                ...
                                <?php if(
                                    ...
                                    <td class="align-middle text-center text-lg text-nowrap">
                                        <?php if(isset($config['booktour']['excel']) && $config['booktour']['excel'] == true) { ?>
                                            <a class="text-primary mr-2" href="index.php?com=excel&id=<?=$items[$i]['id']?>" title="Xuất file excel"><i class="far fa-file-excel"></i></a>
                                        <?php } ?>
                                        <?php if(isset($config['booktour']['word']) && $config['booktour']['word'] == true) { ?>
