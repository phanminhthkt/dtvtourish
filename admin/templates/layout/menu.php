...
<aside class="main-sidebar sidebar-dark-primary elevation-4 text-sm">
    <!-- Logo -->
    <a class="brand-link d-block pt-1 pb-1" href="index.php">
        <img class="brand-image img-responsive" style="max-height: 90%" src="assets/images/logo-idtvn.png" alt="Idt vn">
    </a>
    ...
    <div class="sidebar">
        <nav class="mt-3">
            <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <?php } } } ?>
                <?php if(isset($config['attribute'])) { ?>
                    <?php
                        $none = "";
                        $active = "";
                        $menuopen = "";
                        if(isset($kiemtra) && $kiemtra == true) if($func->check_access('attribute', 'man_list', $k, null, 'phrase-1') && $func->check_access('attribute', 'man', '', $config['attribute'], 'phrase-1')) $none = "d-none";
                        if($com=='attribute')
                        {
                            $active = 'active';
                            $menuopen = 'menu-open';
                    ...
                    ?>
                    <li class="nav-item has-treeview <?=$menuopen?> <?=$none?>">
                        <a class="nav-link <?=$active?>" href="#" title="Quản lý attribute">
                            <i class="nav-icon text-sm fas fa-align-left"></i>
                            <p>
                                Quản lý thuộc tính
                                <i class="right fas fa-angle-left"></i>
                            </p>
                        ...
                        <ul class="nav nav-treeview">
                            <?php foreach($config['attribute'] as $k => $v) { ?>
                                <?php
                                    $none = "";
                                    $active = "";
                                    if(isset($kiemtra) && $kiemtra == true) if($func->check_access('attribute', 'man', $k, null, 'phrase-1')) $none = "d-none";
                                    if($com=='attribute' && $k==$_GET['type']) $active = "active";
                                ?>
                                <?php if(isset($v['list']) && $v['list'] == true) {
                                    $none = "";
                                    $active = "";
                                    if(isset($kiemtra) && $kiemtra == true) if($func->check_access('product', 'man_list', $k, null, 'phrase-1')) $none = "d-none";
                                    if($com=='product' && ($act=='man_list' || $act=='add_list' || $act=='edit_list' || $kind=='man_list') && $k==$_GET['type']) $active = "active"; ?>
                                    <li class="nav-item <?=$none?>"><a class="nav-link <?=$active?>" href="index.php?com=attribute&act=man_list&type=<?=$k?>" title="Danh mục cấp 1"><i class="nav-icon text-sm far fa-caret-square-right"></i><p>Danh mục cấp 1</p></a></li>
                                <?php } ?>
                                <li class="nav-item <?=$none?>">
                                    <a class="nav-link <?=$active?>" href="index.php?com=attribute&act=man&type=<?=$k?>" title="<?=$v['title_main']?>"><i class="nav-icon text-sm far fa-caret-square-right"></i><p><?=$v['title_main']?></p></a>
                                </li>
                            <?php } ?>
                        </ul>
                    </li>
