<?php
	...
	function get_attribute($id=0,$column,$typeAtt)
	{
		global $d, $type;

		if($id)
		{
			$temp = $d->rawQueryOne("select $column from #_product where id = ? and type = ? limit 0,1",array($id,$type));
		}
		$row_attribute = $d->rawQuery("select tenvi, id from #_attribute where type = ? order by stt,id desc",array($typeAtt));
		$str = '<select id="'.$column.'" name="'.$column.'" class="select select2" >';
		$str .= '<option value="0" /> Chọn thuộc tính </option>';
		for($i=0;$i<count($row_attribute);$i++)
		{
			if(isset($temp) && $temp[$column] !=0)
			{
				if($temp[$column] == $row_attribute[$i]['id']) $selected = 'selected="selected"';
				else $selected = '';
			}
			else
			{
			...
			$str .= '<option value="'.$row_attribute[$i]["id"].'" '.$selected.' /> '.$row_attribute[$i]["tenvi"].'</option>';
		}
		$str .= '</select>';
		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if(isset($config['product'][$type]['ngaykhoihanh']) && $config['product'][$type]['ngaykhoihanh'] == true) { ?>
		                                    <div class="form-group">
		                                        <label for="ngaykhoihanh<?=$k?>">Ngày khởi hành (<?=$k?>):</label>
		                                        <input type="text" class="form-control" name="data[ngaykhoihanh<?=$k?>]" id="ngaykhoihanh<?=$k?>" placeholder="Ngày khởi hành (<?=$k?>)" value="<?=@$item['ngaykhoihanh'.$k]?>">
		                                    </div>
		                                	<?php } ?>
		                                	<?php if(isset($config['product'][$type]['phuongtien']) && $config['product'][$type]['phuongtien'] == true) { ?>
		                                    <div class="form-group">
		                                        <label for="phuongtien<?=$k?>">Phương tiện (<?=$k?>):</label>
		                                        <input type="text" class="form-control" name="data[phuongtien<?=$k?>]" id="phuongtien<?=$k?>" placeholder="Phương tiện (<?=$k?>)" value="<?=@$item['phuongtien'.$k]?>">
		                                    </div>
		                                	<?php } ?>
		                                	<?php if(isset($config['product'][$type]['thoigiandi']) && $config['product'][$type]['thoigiandi'] == true) { ?>
		                                    <div class="form-group">
		                                        <label for="thoigiandi<?=$k?>">Thời gian đi (<?=$k?>):</label>
		                                        <input type="text" class="form-control" name="data[thoigiandi<?=$k?>]" id="thoigiandi<?=$k?>" placeholder="Thời gian đi (<?=$k?>)" value="<?=@$item['thoigiandi'.$k]?>">
		                                    </div>
		                                	<?php } ?>
		                                	<?php if(isset($config['product'][$type]['thongtinhotro']) && $config['product'][$type]['thongtinhotro'] == true) { ?>
		                                    <div class="form-group">
		                                        <label for="thongtinhotro<?=$k?>">Thông tin hỗ trợ (<?=$k?>):</label>
		                                        <input type="text" class="form-control" name="data[thongtinhotro<?=$k?>]" id="thongtinhotro<?=$k?>" placeholder="Thông tin hỗ trợ (<?=$k?>)" value="<?=@$item['thongtinhotro'.$k]?>">
		                                    </div>
		                                	<?php } ?>
		                                    ...
		                                    <?php if(isset($config['product'][$type]['noidung']) && $config['product'][$type]['noidung'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="noidung<?=$k?>">Tính năng (<?=$k?>):</label>
		                                    ...
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['lichtrinh']) && $config['product'][$type]['lichtrinh'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="lichtrinh<?=$k?>">Lịch trình (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['lichtrinh_cke']) && $config['product'][$type]['lichtrinh_cke'] == true)?'form-control-ckeditor':''?>" name="data[lichtrinh<?=$k?>]" id="lichtrinh<?=$k?>" rows="5" placeholder="Thông tin đặt hàng (<?=$k?>)"><?=htmlspecialchars_decode(@$item['lichtrinh'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>

		                                    <?php if(isset($config['product'][$type]['quydinh']) && $config['product'][$type]['quydinh'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="quydinh<?=$k?>">Quy định (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['quydinh_cke']) && $config['product'][$type]['quydinh_cke'] == true)?'form-control-ckeditor':''?>" name="data[quydinh<?=$k?>]" id="quydinh<?=$k?>" rows="5" placeholder="Quy định (<?=$k?>)"><?=htmlspecialchars_decode(@$item['quydinh'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                </div>
		                            <?php } ?>
        	...
        	<div class="<?=$colRight?>">
				...
				<div class="card card-primary card-outline text-sm">
		            ...
		            <div class="card-body">
	            		<div class="form-group-category row">
					    	<div class="form-group col-xl-6 col-sm-4">
			                    <label class="d-block" for="id_khoihanh">Nơi khởi hành:</label>
			                    <?=get_attribute(@$id, 'id_khoihanh', 'khoi-hanh')?>
			                </div>
			                <div class="form-group col-xl-6 col-sm-4">
			                    <label class="d-block" for="id_noiden">Nơi đến:</label>
			                    <?=get_attribute(@$id, 'id_noiden', 'noi-den')?>
			                </div>
			                <div class="form-group col-xl-6 col-sm-4">
			                    <label class="d-block" for="id_songaydi">Số ngày đi:</label>
			                    <?=get_attribute(@$id, 'id_songaydi', 'so-ngay-di')?>
			                </div>
						</div>
		            </div>
		        </div>
                ...
                <div class="row">
	                <?php if(isset($config['product'][$type]['ma']) && $config['product'][$type]['ma'] == true) { ?>
	                    <div class="form-group col-md-4">
	                        <label class="d-block" for="masp">Mã tour:</label>
	                ...
	                <?php if(isset($config['product'][$type]['giatreem']) && $config['product'][$type]['giatreem'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        ...
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price" name="data[giatreem]" id="giatreem" placeholder="Giá trẻ em" value="<?=@$item['giatreem']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['socho']) && $config['product'][$type]['socho'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="socho">Số chỗ:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price" name="data[socho]" id="socho" placeholder="Số chỗ" value="<?=@$item['socho']?>">
	                        </div>
	                    </div>
				    <?php } ?>
