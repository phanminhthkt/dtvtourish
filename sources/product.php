<?php  
	...
	if($id!='')
	{
		$reasons = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen from #_news where type = ? and hienthi > 0 order by stt,id asc",array('li-do'));

		$tourPromotion = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen from #_product where type = ? and hienthi > 0 and noibat1 > 0 order by stt,id asc",array('tour'));
		/* Lấy sản phẩm detail */
		...
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen, giamoi, gia,gianguoilon,ngaykhoihanh$lang,id_khoihanh, giakm, id from #_product where $where order by stt,id desc $limit";
