<?php
	class Functions
	{
		...
		public function get_attribute_input($id_get,$nameElement,$type,$text){ 
			global $lang,$d;
			$attributes = $d->rawQuery("select ten$lang,id from #_attribute where type='".$type."' order by stt asc");

			$str ='<select id="'.$nameElement.'" name="'.$nameElement.'" class="form-control">';
			$str.='<option value="0">'.$text.'</option>';
			foreach($attributes as $v){
				if(isset($id_get) && $id_get!='' && $id_get == $v['id']){$select ='selected';}else{$select ='';}
				$str.='<option value='.$v['id'].' '.$select.'>'.$v['ten'.$lang].'</option>';
			}
			$str.='</select>';
			return $str;
		}
	    ...
	    public function get_product_list($array,$class,$com){ 
			global $lang,$d;
			?>
			<div class="product-all row"> 
			<?php 	foreach($array as $v){ 
				$cat = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_product_cat where type = ? and hienthi > 0 and id_list=? order by stt,id asc",array('san-pham',$v['id']));
			?>
				<div class="col-md-12 col-sm-12 col-12">
		            <div class="product-list-in transition">
		            	<h3><a href="<?=$v['tenkhongdauvi']?>"><?=$v['ten'.$lang]?></a></h3>
		            	<div class="row">
		            		<div class="col-md-4 col-sm-12">
		            			<div class="product-list-in_img">
			                		<a href="<?=$v['tenkhongdauvi']?>">
					                    <img class="transition w-100" onerror="this.src='<?=THUMBS?>/300x230x1/assets/images/noimage.png';" alt="<?=$v['ten'.$lang]?>" src="<?=THUMBS?>/300x230x1/<?=UPLOAD_PRODUCT_L.$v['photo']?>">
					                </a>
				                </div>
		            		</div>
		            		<div class="col-md-8 col-sm-12">
		            			<?php if(count($cat)){ ?>
				                    <div class="list-child__cat transition">
				                        <ul>
				                        	<?php foreach($cat as $vCat){ ?>
				                        	<li><a href="<?=$vCat['tenkhongdauvi']?>"><i class="fa fa-caret-right"></i><?=$vCat['ten'.$lang]?></a></li>
				                        	<?php } ?>
				                        </ul>
				                    </div>
				                <?php } ?>
		...
		public function get_product_cat($array,$class,$com){ 
			global $lang,$d;
			?>
			<div class="product-all row"> 
			<?php 	foreach($array as $v){ 
				$cat = $d->rawQuery("select ten$lang, tenkhongdauvi, tenkhongdauen, id from #_product where type = ? and hienthi > 0 and id_cat=? order by stt,id asc limit 7",array('san-pham',$v['id']));
			?>
				<div class="col-md-12 col-sm-12 col-12">
		            <div class="product-list-in transition">
		            	<h3><a href="<?=$v['tenkhongdauvi']?>"><?=$v['ten'.$lang]?></a></h3>
		            	<div class="row">
		            		...
		            		<div class="col-md-8 col-sm-12">
				                ...
				                <a href="<?=$v['tenkhongdauvi']?>" class="see-more"> Xem tất cả >></a>
				                <?php } ?>
		            		</div>
		            	</div>
		            </div> 
		            ...
		            <div class="product transition">
		                <a href="<?=$v['tenkhongdauvi']?>">
		                	<div class="product_img">
			                    <img class="transition w-100" onerror="this.src='<?=THUMBS?>/570x460x1/assets/images/noimage.png';" alt="<?=$v['ten'.$lang]?>" src="<?=THUMBS?>/570x460x1/<?=UPLOAD_PRODUCT_L.$v['photo']?>">
			                    <div class="gia-product">
			                    	<span><?=(isset($v['gianguoilon']) && $v['gianguoilon']!=0 ) ? $this->format_money($v['gianguoilon'],'đ') : 'Liên hệ'?></span>
			                    </div>
			                </div>
			                <div class="info-product">
		                        <h3><span><?=$v['ten'.$lang]?></span></h3>
			                	<p><?=ngaykhoihanh?>: <span><?=$v['ngaykhoihanh'.$lang]?></span></p>
			                	<p><?=noikhoihanh?>: <span><?=$this->getValueAttribute($v['id_khoihanh'],'tenvi')?></span></p>
			                </div>
		...
		<?php }
		public function get_news2($array,$class,$com){ 
			global $lang;
			?>
			<div class="row"> 
			<?php 	foreach($array as $v){ ?>
				<div class="col-md-3 col-12 col-sm-6">
		            <a class="visa text-decoration-none w-clear" href="<?=$v['tenkhongdauvi']?>" title="<?=$v['ten'.$lang]?>">
			            <p class="pic-visa scale-img"><img onerror="this.src='<?=THUMBS?>/285x220x1/assets/images/noimage.png';" src="<?=THUMBS?>/285x220x1/<?=UPLOAD_NEWS_L.$v['photo']?>" alt="<?=$v['ten'.$lang]?>"></p>
			            <div class="info-visa transition">
			                <h3 class="name-visa"><?=$v['ten'.$lang]?></h3>
			            </div>
			        </a>
		        </div>              
				<?php } ?>
		...
		<?php }
		public function get_download($array,$class,$com){ 
			global $lang;
			?>
			<div class="row"> 
			<?php 	foreach($array as $v){ ?>
				<div class="col-md-3 col-12 col-sm-6">
		            <a class="download text-decoration-none w-clear" download href="<?=UPLOAD_NEWS_L.$v['taptin']?>" title="<?=$v['ten'.$lang]?>">
			            <p class="pic-download hover_bright scale-img"><img onerror="this.src='<?=THUMBS?>/285x200x1/assets/images/noimage.png';" src="<?=THUMBS?>/285x200x1/<?=UPLOAD_NEWS_L.$v['photo']?>" alt="<?=$v['ten'.$lang]?>"></p>
			            <div class="info-download">
			                <h3 class="name-news"><?=$v['ten'.$lang]?></h3>
			                <p><i class="fa fa-file-download"></i> Tải file</p>
			            </div>
			        </a>
		        </div>              
				<?php } ?>
